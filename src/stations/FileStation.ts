...
import { resolveFileStationError } from '../lib/errors.js';

const API_PREFIXES = ['SYNO.FileStation'];
const SESSION_NAME = 'FileStation';

export class FileStation extends AuthenticatedAPI {
  ...
  override resolveError(code: number, api?: string): string {
    return resolveFileStationError(code, api || '');
  }
}
